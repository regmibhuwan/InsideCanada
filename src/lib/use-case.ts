...
export function useCase() {
  ...
  const fetchCase = useCallback(async () => {
    try {
      ...
      let prApplications: PRApplication[] = [];
      try {
        const { data } = await supabase.from("pr_applications").select("*").eq("user_id", user.id).order("created_at", { ascending: false });
        prApplications = (data || []) as PRApplication[];
      } catch {
      ...
      console.error("Failed to fetch user case:", err);
